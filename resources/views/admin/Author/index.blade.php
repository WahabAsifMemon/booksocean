...
<section class="content">
    <div class="box">
        <div class="box-header with-border">
            <h3 class="box-title">
                <a id="activeAllStatus" url="{{ route('author.status.active') }}" class="btn btn-danger btn-xm"><i class="fa fa-eye"></i></a>
                <a id="deactiveAllStatus" url="{{ route('author.status.deactive') }}" class="btn btn-danger btn-xm"><i class="fa fa-eye-slash"></i></a>
            ...
            <div class="row">
                <div class="col-sm-6">
                    <span style="display:block;font-size:15px;line-height:34px;margin:20px 0;">
                        Showing {{($authors->currentpage()-1)*$authors->perpage()+1}} to  {{ $authors->currentpage()*(($authors->perpage() < $authors->total()) ? $authors->perpage(): $authors->total())}} of {{ $authors->total()}} entries
                    </span>
                </div>
...
<script>
    $(document).ready(function() {
        ...
        $(".singleStatus").on('click', function(event) {
            ...
            self.html('<img src="/assets/images/ajax-loader.gif">')
            $.ajax({
                url: url,
                type: 'PUT',
            })
